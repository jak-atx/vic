...
if [[ $DRONE_BRANCH == "master" || $DRONE_BRANCH == "releases/"* ]] && [[ $DRONE_REPO == "vmware/vic" ]] && [[ $DRONE_BUILD_EVENT == "push" ]]; then
    ...
    pabot --processes 3 --removekeywords TAG:secret --exclude skip tests/test-cases
    ...
    pabot --processes 3 --removekeywords TAG:secret --suite Group11-Upgrade --suite 7-01-Regression tests/test-cases
    ...
    pabot --processes 3 --removekeywords TAG:secret --suite $testsuite --suite 7-01-Regression tests/test-cases
else
    echo "Running regressions"
    pabot --processes 3 --removekeywords TAG:secret --exclude skip --include regression tests/test-cases
fi
